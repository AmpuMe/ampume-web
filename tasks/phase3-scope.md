...
- [X] C7. Brand filter: remove or convert to dropdown (to avoid long list in future). Per call, agreed to remove for now.
...
- [X] C11. Fabric variant section: add tooltip explaining fabric options AND/OR link to dedicated fabric explanation section
- [X] C12. Consider moving fabric section higher on page (before sizing, per call — "Probably before sizing and fit... because it's purchase dependent")
- [X] C13. Add "Fabric" anchor link to the anchor bar (for products where fabric is an option)
- [x] C14. Remove "Choose Your Fabric" label → "Fabric Options"
- [X] C15. Product gallery: remove third product image — THIS IS FOR ALPHA CLASSIC BK products specifically. Alex said "This image is not applicable. This one you can remove." while looking at Willowwood BK product page. The third image is for a different product/profile.
